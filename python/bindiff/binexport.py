...
def _load_pb2():
    ...
    try:
        ...
        # protobuf's own VersionError, which derives from Exception and not
        # from ImportError, so the handler above never saw it. It is raised
        # from inside the generated module when the runtime is older than the
        # protoc that stamped it, and the traceback points at _pb rather than
        # at anything the reader can act on.
        #
        # This bites in IDA specifically: the bindings are generated by the
        # protoc the CMake build produces, while IDA ships its own interpreter
        # with its own protobuf, and nothing makes the two agree. pyproject's
        # floor governs a pip install, which a plugin loaded from a directory
        # never performs.
        raise RuntimeError(_explain_protobuf(exc)) from exc
    return binexport2_pb2


def _explain_protobuf(exc: Exception) -> str:
    """Turns "gencode 7.35.1 runtime 6.33.1" into something to type.

    Names the interpreter as well as the version: in IDA the one that must be
    upgraded is IDA's, not whichever python is on the PATH, and installing
    into the wrong one is the obvious next mistake.
    """
    import sys
    ...
    try:
        ...
        runtime = "unknown"
    required = _gencode_version() or "the version that generated them"
    return (
        f"The protobuf runtime in this interpreter ({runtime}) is older than "
        f"the generated bindings ({required}).\n\n"
        f"{exc}\n\n"
        f"Install a newer protobuf into *this* interpreter:\n"
        f"  {sys.executable} -m pip install 'protobuf>={required}'\n\n"
        f"Inside IDA that is IDA's own interpreter, not the one on your PATH.")


def _gencode_version() -> str:
    """The protoc version stamped into the bindings, or "" if unreadable.

    Read from the module rather than parsed out of the exception text, which
    protobuf is free to reword.
    """
    try:
        from bindiff._pb import binexport2_pb2  # noqa: F401
    except Exception:
        pass
    try:
        import re
        ...
        source = _Path(__file__).with_name("_pb") / "binexport2_pb2.py"
        text = source.read_text(encoding="utf-8")
        match = re.search(
            r"ValidateProtobufRuntimeVersion\(\s*[^,]+,\s*"
            r"(\d+),\s*(\d+),\s*(\d+)", text)
