...
**`ida_plugin/session.py` is the one owner of every fact the UI shows** — state (idle, comparing, open clean/edited/partial), the open result's identity and counts, the selection, the unsaved-edit count, and the port ledger. Views subscribe to its signals and call its methods; `session.can(action)` is evaluated on every ask and never cached. Every IDA action is registered `AST_ENABLE_ALWAYS` because `AST_DISABLE_ALWAYS` is recorded once and never re-asked, which is how three views spent a session greyed out. Trust (`trust.py`) is a plugin-side verdict from similarity, block coverage and a name-string class table; the `.BinDiff` schema is not extended, so the by-hand set and the ledger are session-only.

The workbench's run strip (`ida_plugin.workbench.RunStrip`) renders it: it implements the panel protocol `DiffRun` expects — `start`, `update_progress`, `finish` — and is part of the one dock tab rather than a window of its own, not `show_wait_box`, which is modal and would give back the responsiveness the whole out-of-process design buys. The Compare button and the bar are two pages of one `QStackedWidget`, so a run replaces the control that started it and cannot be started twice. Exports get Qt's indeterminate bar (`setRange(0, 0)`) because idalib's auto-analysis reports nothing to make a fraction from; the pipeline weights the two exports at 60% of the bar (`_EXPORT_SHARE`) since they are much the slower half. Nothing here calls `processEvents()` — a script doing its work on the UI thread needs that (eidolon's pattern) but the work is in another process, so the UI thread is already free and re-entering the event loop by hand would only invite reentrancy.

**Cancelling asks; it does not kill.** The launcher writes `cancel` on the worker's stdin (`--cancel-on-stdin`, passed only when a `cancel` event was given, so a worker run by hand keeps its stdin). The worker answers with a `cancelling` progress record, returns False from its own engine callback, and the engine's cancel-to-partial writes the matches it already had — which crosses the process boundary as the same `.BinDiff` and the same JSON line a complete run uses. Measured on `fixtures/benchmark`: **13.9s → 0.94s, keeping 729 of 10230 matches**; terminating the worker instead would have kept none. stdin rather than a signal because SIGINT cannot be delivered to one child on Windows. The ask is made from the run strip's Cancel, beside the bar that took the Compare button's place — the same control, so there is no second window to find and none to lose track of.
